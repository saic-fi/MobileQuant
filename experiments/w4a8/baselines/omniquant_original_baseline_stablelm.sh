...
CUDA_VISIBLE_DEVICES=0 python ptq/smoothquant.py --hf_path checkpoints/baselines/stablelm-2-1_6b --original_omniquant --alpha 0.6
CUDA_VISIBLE_DEVICES=0 python ptq/generate_act_range.py --hf_path checkpoints/baselines/stablelm-2-1_6b


EPOCHS=20
NSAMPLES=128
CKPT=checkpoints/baselines/stablelm-2-1_6b
OUTPUT_DIR=results/stablelm-2-1_6b-omniquant-orig-w4a8
BATCH_SIZE=1
LET_LR=1e-3
LET_MIN_LR=5e-4
LWC_LR=5e-3
...
CUDA_VISIBLE_DEVICES=0 python ptq/mobilequant.py --lwc --let --tasks wikitext \
  ...
  --batch_size ${BATCH_SIZE} --weight_bitwidth 4 --weight_is_per_channel --original_omniquant
