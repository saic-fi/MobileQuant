...
def assign_layers_to_gpus(layers: List[nn.Module]):
    ...
    for i, layer in enumerate(layers):
        if i == len(layers) - 1:
            # mv output to the same device as the input
        ...
        if prev_gpu_id is None:
            ...
            # use the same gpu as the prev layer if possible
...
def assign_layers_to_gpus_evenly(layers: List[nn.Module]):
    layer_gpu_map = {}
    prev_gpu_id = None
    weight_num = 0
    for module in layers:
    ...
    all_gpus = get_gpu_memory()
    num_layers = len(layers)
    bin_size = num_layers // len(all_gpus)
    ...
    for i, layer in enumerate(layers):
        if i == len(layers) - 1:
            ...
            print(f"map last layer {i} to gpu {layer_gpu_map[layer]}")
            continue
        gpu_id = min(i // bin_size, len(all_gpus) - 1)
        layer_gpu_map[layer] = gpu_id
        available_gpus = get_gpu_memory()
        layer.to(f"cuda:{gpu_id}")
        layer.device = f"cuda:{gpu_id}"
...
def map_layers_to_multi_gpus(layers):
    ...
    layer_gpu_map = assign_layers_to_gpus_evenly(layers)
